...
config.kv_namespaces = (config.kv_namespaces ?? []).filter(ns => ns.binding !== 'SESSION');
if (config.previews?.kv_namespaces) {
  config.previews.kv_namespaces = config.previews.kv_namespaces.filter(ns => ns.binding !== 'SESSION');
}
